...
const WeatherCard = ({
  ...
  return (
    <div className="rounded-lg shadow-lg bg-white p-4 space-y-2 text-center w-[300px]">
      ...
      <div>💨 Wind Max: {Math.round(windSpeed)} mph</div>
      <div> 🌧️ Chance: {Math.round(precipChance)}%</div>
      <div>🌧️ Total: {Math.round(precipSum)} mm</div>
    </div>
  );
};
